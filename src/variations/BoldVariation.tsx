      ...
      <section id="about" className="py-16 md:py-32 border-b border-white/10 relative bg-[#050505]">
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          <div className="space-y-6">
            ...
            {/* Compact Stats Row */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-white/10 border border-white/10 mt-16">
              {about.stats.map((stat, i) => (
                <FadeIn key={stat.label} delay={0.2 + (i * 0.1)} className="bg-[#050505] p-6 md:p-8 group hover:bg-white/[0.02] transition-colors flex flex-col justify-center">
                  <div className="text-xs md:text-sm font-mono uppercase tracking-widest text-zinc-500 mb-3">
                    <Reveal>{stat.label}</Reveal>
                  </div>
                  <div className="text-3xl md:text-4xl font-black tracking-tighter text-white group-hover:text-emerald-400 transition-colors duration-500">
                        ...
                        <div className={`lg:col-span-7 relative ${isEven ? 'lg:order-1' : 'lg:order-2'}`}>
                          <a href={project.link} target="_blank" rel="noopener noreferrer" className="block relative overflow-hidden aspect-[16/10] bg-white/5 border border-white/10 group/img">
                            <img 
                              src={project.image} 
                              alt={project.title}
                              className="w-full h-full object-cover grayscale opacity-70 transition-all duration-1000 group-hover/img:scale-105 group-hover/img:grayscale-0 group-hover/img:opacity-100"
                              referrerPolicy="no-referrer"
                            />
